...
#               Language: Python                                #
#               Projet: Similarity of sentences with Spacy      #
#               Version: V1                                     #
...
if __name__ == "__main__":
    print("\n\tSIMILARITY ANALYSE WITH SPACY LEMMATIZATION\n")
    ...
    lemma1 = [t.lemma_ for t in token1]
    lemma2 = [t.lemma_ for t in token2]
    ...
    for word in corpus:
        ...
        infValue1 += (lemma1.count(word) * lemma1.count(word))
        infValue2 += (lemma2.count(word) * lemma2.count(word))
    infValue = sqrt(infValue1) * sqrt(infValue2)

    similarity = supValue / infValue
    print("\nThe result of similarity with spacy lemmatization is:\n")
